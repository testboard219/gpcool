...
if($search){
  $statement= $pdo->prepare('SELECT * FROM approvetableone  WHERE fname LIKE :fname ORDER BY date DESC');
 $statement->bindValue(":fname","%$search%");


}else{
$statement= $pdo->prepare('SELECT * FROM approvetableone ORDER BY date DESC');
}
$statement->execute();
  ...
  <?php foreach ($product as $i => $products):?>
    <tr>
     ...
     <!--- #begining of test run--->
      <form action='approved.php' method='post' style='display:inline-block;'>
